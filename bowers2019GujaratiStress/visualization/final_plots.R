...
pFemalemin <- ggplot(gujinit[gujinit$Gender=="female", ] , aes(x=as.integer(SyllPos), y=F0_min-min_min, color=Vowel)) +
    geom_point(shape=1) +
...
pMalemin <- ggplot(gujinit[gujinit$Gender=="male", ] , aes(x=as.integer(SyllPos), y=F0_min-min_min, color=Vowel)) +
    #geom_point(shape=1) +
...
#pdf("/home/sautedman/sideProjects/gujarati/stress/flashStickBackup/visualization/grayscale/extrapolatedf0.pdf")
